...
= 特定のページでだけフローティングツールを表示したい =

* 管理画面の「表示制御」で「ブロック設置ページのみ表示」を選択
* 表示したいページの編集画面で「Floating Tools Settings」ブロックを追加
* ブロックが配置されたページでのみフローティングツールが表示されます

= ブロックを追加したのにフローティングツールが表示されない =

* 管理画面の「表示制御」設定を確認してください
* 「すべてのページに表示」を選択している場合は、ブロックの有無に関係なく表示されます
* 「ブロック設置ページのみ表示」を選択している場合は、ブロックが必要です
